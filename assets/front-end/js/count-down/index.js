var CountDown = function($scope, $) {
    var $coundDown = $scope.find(".pt-countdown-wrapper").eq(0),
        $countdown_id =
    ...
    jQuery(document).ready(function($) {
        "use strict";
        var countDown = $("#pt-countdown-" + $countdown_id);

        countDown.countdown({
            end: function() {
                if ($expire_type == "text") {
                    countDown.html(
                        '<div class="pt-countdown-finish-message"><h4 class="expiry-title">' +
                            $expiry_title +
                            "</h4>" +
                            '<div class="pt-countdown-finish-text">' +
                            $expiry_text +
                            "</div></div>"
                    );
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/pt-count-down.default",
        CountDown
    );
});
