...
jQuery(document).ready(function($){
	var timelineBlocks = $('.pt-content-timeline-block'),
		offset = 0.8;
	...
	function hideBlocks(blocks, offset) {
		blocks.each(function(){
			( $(this).offset().top > $(window).scrollTop()+$(window).height()*offset ) && $(this).find('.pt-content-timeline-img, .pt-content-timeline-content').addClass('is-hidden');
		});
	}

	function showBlocks(blocks, offset) {
		blocks.each(function(){
			( $(this).offset().top <= $(window).scrollTop()+$(window).height()*offset && $(this).find('.pt-content-timeline-img').hasClass('is-hidden') ) && $(this).find('.pt-content-timeline-img, .pt-content-timeline-content').removeClass('is-hidden').addClass('');
		});
	}
	...
	function f(e){
	    getElementsInArea(e, {
	        elements    : document.querySelectorAll('.pt-content-timeline-block'),
...
(function($) {
	'use strict';
	window.pt_mailchimp_subscribe = function( formId, apiKey, listId, buttonText, successMsg, loadingText ) {
		$( '#'+formId ).on('submit', function(e) {
			e.preventDefault();
			var self = $(this);

			self.find('.pt-mailchimp-subscribe').addClass( 'button--loading' );
			self.find('.pt-mailchimp-subscribe span').html( loadingText );
			$.ajax({
				...
				success: function(data) {
					if( data != 'error' ) {
						self.find('.pt-mailchimp-fields-wrapper').after( '<div class="pt-mailchimp-message"><p>'+successMsg+'</p></div>' );
						self.find('input[type=text], input[type=email], textarea').val('');
						self.find('.pt-mailchimp-subscribe').removeClass( 'button--loading' );
						self.find('.pt-mailchimp-subscribe span').html( buttonText );
					} else {
						self.find('.pt-mailchimp-fields-wrapper').after( '<div class="pt-mailchimp-message"><p>Something goes wrong, Try again later.</p></div>' );
						self.find('.pt-mailchimp-subscribe').removeClass( 'button--loading' );
						self.find('.pt-mailchimp-subscribe span').html( buttonText );
					}
				}
...
(function ($) {
    ...
    var OnePageNavHandler = function ($scope, $) {
        var onepage_nav_elem                = $scope.find('.pt-one-page-nav').eq(0);
        ...
        var $section_id      = '#' + onepage_nav_elem.data('section-id'),
            ...
            $target_dot      = $section_id + ' .pt-one-page-nav-item a',
            $nav_item        = $section_id + ' .pt-one-page-nav-item',
            $active_item     = $section_id + ' .pt-one-page-nav-item.active';
        ...
        $( $target_dot ).on( 'click', function(e) {
            ...
            $( $section_id + ' .pt-one-page-nav-item' ).removeClass( 'active' );
        ...
        function updateDot() {
            $('.elementor-section').each(function(){
                ...
                if ( ( $this.offset().top - $(window).height()/2 < $(window).scrollTop() ) && ( $this.offset().top >= $(window).scrollTop() || $this.offset().top + $this.height() - $(window).height()/2 > $(window).scrollTop() ) ) {
                    $( $section_id + ' .pt-one-page-nav-item a[data-row-id="'+$this.attr('id')+'"]' ).parent().addClass('active');
                } else {
                    $( $section_id + ' .pt-one-page-nav-item a[data-row-id="'+$this.attr('id')+'"]' ).parent().removeClass('active');
                }
            });
        }
        if ( $scroll_wheel == 'on' ) {
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/pt-one-page-nav.default', OnePageNavHandler);
    });
...
(function ($) {
    $(document).ready(function ($) {
        ...
        $input.on('keyup', function (e) {
            ...
            var $key = $(this).val(),
                $nonce = $(this).siblings('#pt_ajax_post_search_nonce').val();

            $.ajax({
                url: localize.ajaxurl,
                type: 'post',
                data: {
                    action: 'pt_ajax_post_search',
                    _nonce: $nonce,
                    key: $key
                },
                success: function (r) {
	...
	$.fn.ptProgressBar = function() {
		...
		$this.one('inview', function() {
			if ($layout == 'line' || $layout == 'line_rainbow') {
				$('.pt-progressbar-line-fill', $this).css({
					'width': $num + '%',
				})
			} else if ($layout == 'half_circle' || $layout == 'half_circle_fill') {
				$('.pt-progressbar-circle-half', $this).css({
					'transform': 'rotate(' + ($num * 1.8) + 'deg)',
				})
			} else if ($layout == 'box') {
				$('.pt-progressbar-box-fill', $this).css({
					'height': $num + '%',
				})
			}

			$('.pt-progressbar-count', $this).prop({
				'counter': 0
			}).animate({
				counter: $num
			}, {
				...
				step: function(counter) {
					if ($layout == 'circle' || $layout == 'circle_fill') {
						...
						$('.pt-progressbar-circle-half-left', $this).css({
							'transform': "rotate(" + rotate + "deg)",
						})
						if (rotate > 180) {
							$('.pt-progressbar-circle-pie', $this).css({
								'clip-path': 'inset(0)'
							})
							$('.pt-progressbar-circle-half-right', $this).css({
								'visibility': 'visible'
							})
						}
					}
...
var AdvGoogleMap = function($scope, $) {
	window.ptHasMapAPI = window.google ? window.google : undefined;

	if (!window.ptHasMapAPI) {
		var $map_class = $scope.find(".pt-google-map").eq(0),
			$map_notice = $scope.find(".google-map-notice").eq(0);

		$map_class.css("display", "none");

		$map_notice.html(
			"Whoops! It' seems like you didn't set Google Map API key. You can set from <b>Elementor > PT Addons for Elementor > Elements > Advanced Google Map (Settings)</b>"
		);
		...
		var $map = $scope.find(".pt-google-map"),
			$thisMap = $("#" + $map.attr("id")),
		...
		var ptMapHeader = new GMaps({
			el: "#pt-google-map-" + $mapID,
			lat: $mapLat,
			lng: $mapLng,
			zoom: $mapZoom,
		...
		if ($mapTheme != "") {
			ptMapHeader.addStyle({
				styledMapName: "Styled Map",
				styles: JSON.parse($mapTheme),
				mapTypeId: "map_style"
			});

			ptMapHeader.setStyle("map_style");
		}
				...
				GMaps.geocode({
					...
					callback: function(results, status) {
						if (status == "OK") {
							...
							ptMapHeader.setCenter(latlng.lat(), latlng.lng());
							ptMapHeader.addMarker({
								lat: latlng.lat(),
								lng: latlng.lng(),
		...
		if ("marker" == $mapType) {
			...
			if ($data.length > 0) {
				var MarkersMap = new GMaps({
					el: "#pt-google-map-" + $mapID,
					lat: $data[0].pt_google_map_marker_lat,
					lng: $data[0].pt_google_map_marker_lng,
					zoom: $mapZoom,
					streetViewControl: $map_streeview_control,
					mapTypeControl: $map_type_control,
					zoomControl: $map_zoom_control,
				...
				MarkersMap.setCenter(
					$data[0].pt_google_map_marker_lat,
					$data[0].pt_google_map_marker_lng
				);
				...
				$data.forEach(function($marker) {
					if ($marker.pt_google_map_marker_content != "") {
						var infoWindowHolder = {
							content: $marker.pt_google_map_marker_content
						};
					} else {
						var infoWindowHolder = "";
					}

					if ($marker.pt_google_map_marker_icon_enable == "yes") {
						var iconHolder = {
							url: $marker.pt_google_map_marker_icon.url,
							scaledSize: new google.maps.Size(
								$marker.pt_google_map_marker_icon_width,
								$marker.pt_google_map_marker_icon_height
							) // scaled size
						};
					} else {
						var iconHolder = "";
					...
					MarkersMap.addMarker({
						lat: parseFloat($marker.pt_google_map_marker_lat),
						lng: parseFloat($marker.pt_google_map_marker_lng),
						title: $marker.pt_google_map_marker_title,
						infoWindow: infoWindowHolder,
						icon: iconHolder
					});
				});
		...
		if ("static" == $mapType) {
			...
			if ($data.length > 0) {
				$data.forEach(function($marker) {
					markersHolder.push({
						lat: parseFloat($marker.pt_google_map_marker_lat),
						lng: parseFloat($marker.pt_google_map_marker_lng),
						color: $marker.pt_google_map_marker_icon_color
					});
				});
			}

			var ptStaticMapUrl = GMaps.staticMapURL({
			...
			$("<img />")
				.attr("src", ptStaticMapUrl)
				.appendTo("#pt-google-map-" + $mapID);
		} // End of static map
		...
		if ("polyline" == $mapType) {
			...
			$polylines_data.forEach(function($polyline) {
				$pt_polylines.push([
					parseFloat($polyline.pt_google_map_polyline_lat),
					parseFloat($polyline.pt_google_map_polyline_lng)
				]);
			});

			var path = JSON.parse(JSON.stringify($pt_polylines));

			var ptPolylineMap = new GMaps({
				el: "#pt-google-map-" + $mapID,
				lat: path[0][0],
				lng: path[0][1],
				zoom: $mapZoom
			...
			$data.forEach(function($marker) {
				...
				ptPolylineMap.addMarker({
					lat: $marker.pt_google_map_marker_lat,
					lng: $marker.pt_google_map_marker_lng,
					title: $marker.pt_google_map_marker_title,
					infoWindow: infoWindowHolder,
					icon: iconHolder
				});
			...
			if ($mapTheme != "") {
				ptPolylineMap.addStyle({
					styledMapName: "Styled Map",
					styles: JSON.parse($mapTheme),
					mapTypeId: "polyline_map_style"
				});

				ptPolylineMap.setStyle("polyline_map_style");
			}
		} // End of polyline map
		...
		if ("panorama" == $mapType) {
			var ptPanorama = GMaps.createPanorama({
				el: "#pt-google-map-" + $mapID,
				lat: $mapPanoramaLat,
				lng: $mapPanoramaLng
			});
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-google-map.default",
		AdvGoogleMap
	);
});
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.default",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-one",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-two",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-three",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-four",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-five",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-six",
		AdvancedMenu
	);
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-advanced-menu.skin-seven",
		AdvancedMenu
	);
});
...
/* 40. Global Tooltip
/*=================================*/
var ptGlobalTooltip = function($scope, $) {
    var target = $scope,
    ...
    if (editMode) {
        ...
        $.each(editorElements.models, function(index, elem) {
                        ...
                        $.each(subSec.attributes.elements.models,function(index,subCol){
                            $.each(subCol.attributes.elements.models, function(ind, subWidget) {
                                ...
                                if ( sectionId == subWidget.id ) {
                                    ...
                                    settings.switch = sectionData["pt_tooltip_section_enable"];
                                    settings.content = sectionData["pt_tooltip_section_content"];
                                    settings.position = sectionData["pt_tooltip_section_position"];
                                    settings.animation = sectionData["pt_tooltip_section_animation"];
                                    settings.arrow = sectionData["pt_tooltip_section_arrow"];
                                    settings.arrowType = sectionData["pt_tooltip_section_arrow_type"];
                                    settings.duration = sectionData["pt_tooltip_section_duration"];
                                    settings.delay = sectionData["pt_tooltip_section_delay"];
                                    settings.size = sectionData["pt_tooltip_section_size"];
                                    settings.trigger = sectionData["pt_tooltip_section_trigger"];

                                    settings.distance = sectionData["pt_tooltip_section_distance"];
                                    settings.maxWidth = sectionData["pt_tooltip_section_width"];

                                    if ( settings.switch == "yes" ) {
                                        target.addClass("pt-section-tooltip");
                                        generateTooltip();
                                    } else {
                                        target.removeClass("pt-section-tooltip");
                                    }
            ...
            function generateTooltip() {
                target.attr("id", "pt-section-tooltip-" + sectionId);
...
jQuery(window).on('elementor/frontend/init', function () {
    elementorFrontend.hooks.addAction( 'frontend/element_ready/widget', ptGlobalTooltip );
});
...
var contentTimelineHandler = function($scope, $) {
	var contentBlock = $(".pt-content-timeline-block");
	...
	$(window).on("scroll", function() {
		contentBlock.each(function() {
			if ($(this).find(".highlight")) {
				...
				$(this)
					.find(".pt-content-timeline-inner")
		...
		if (this.oldScroll > this.scrollY == false) {
			...
			// Scroll Down
			$(".pt-content-timeline-block.highlight")
				.prev()
				.find(".pt-content-timeline-inner")
				.removeClass("pt-muted")
				.addClass("pt-highlighted");
		} else if (this.oldScroll > this.scrollY == true) {
			this.oldScroll = this.scrollY;
			// Scroll Up
			$(".pt-content-timeline-block.highlight")
				.find(".pt-content-timeline-inner")
				.addClass("pt-prev-highlighted");
			$(".pt-content-timeline-block.highlight")
				.next()
				.find(".pt-content-timeline-inner")
				.removeClass("pt-highlighted")
				.removeClass("pt-prev-highlighted")
				.addClass("pt-muted");
		}
	});
};
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-content-timeline.default",
...
var CounterHandler = function($scope, $) {
	var counter_elem = $scope.find(".pt-counter").eq(0),
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-counter.default",
		CounterHandler
	);
});
...
window.enableProSorter = function($, $_this) {
    ...
    if( true == $enable_table ) $("#pt-data-table-"+$id).tablesorter();
    if( $enable_table != true ) {
        $('table#pt-data-table-'+$id+' .sorting').addClass('sorting-none');
        $('table#pt-data-table-'+$id+' .sorting_desc').addClass('sorting-none');
        $('table#pt-data-table-'+$id+' .sorting_asc').addClass('sorting-none');
    }
};
var DynamicFilterableGallery = function($scope, $) {
    var $gallery = $('.pt-filter-gallery-container', $scope),
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/pt-dynamic-filterable-gallery.default",
...
var ImageHotspotHandler = function($scope, $) {
    $(".pt-hot-spot-tooptip").each(function() {
        var $position_local = $(this).data("tooltip-position-local"),
            ...
            $arrow =
                $(this).data("pt-tooltip-arrow") === "yes" ? true : false,
    ...
    $(".pt-hot-spot-wrap").on("click", function(e) {
        e.preventDefault();
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/pt-image-hotspots.default",
        ImageHotspotHandler
    );
});
...
var ImageScroller = function($scope, $) {
	$(".pt-image-scroller-hover", $scope).hover(
		function() {
			if ($(this).hasClass("pt-image-scroller-vertical")) {
			...
			} else if ($(this).hasClass("pt-image-scroller-horizontal")) {
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-image-scroller.default",
		ImageScroller
	);
});
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-image-comparison.default",
...
var InstagramGallery = function($scope, $) {
    if (!isEditMode) {
        // init isotope
        $instagram_gallery = $(".pt-insta-grid", $scope).isotope({
            itemSelector: ".pt-insta-feed",
            percentPosition: true,
            columnWidth: ".pt-insta-box"
        });
    ...
    // ajax load more
    $(".pt-load-more-button", $scope).on("click", function() {
        $this = $(this);
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // append items
                $(".pt-insta-grid", $scope).append($html);
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/pt-instafeed.default",
        InstagramGallery
    );
});
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-interactive-card.default",
		InteractiveCard
	);
});
...
var LearnDash = function ($scope, $) {
    ...
    var card = $scope.find('.pt-3d-hover .pt-learn-dash-course'),
        wrap = $scope.find('.pt-learndash-wrapper'),
        hover3D = (wrap.data('3d-hover') !== undefined) ? 'true' : 'false';

    if (!isEditMode) {
        var $settings = {
            itemSelector: ".pt-learn-dash-course",
            percentPosition: true,
            masonry: {
                columnWidth: ".pt-learn-dash-course"
            }
        };

        // init isotope
        $ld_gallery = $(".pt-learndash-wrapper", $scope).isotope($settings);
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-learn-dash-course-list.default",
		LearnDash
	);
});
/*=================================*/
...
var LightBox = function($scope, $) {
	var $lightBox = $scope.find(".pt-lightbox-wrapper").eq(0),
	...
	if ("pt-lightbox-popup-fullscreen" == $popup_layout) {
		var win_height = $(window).height() - 20;
		$(".pt-lightbox-container.content-type-image-now").css({
			"max-height": win_height + "px",
			"margin-top": "10px"
		});
	}

	if ("pt_lightbox_trigger_exit_intent" == $trigger) {
		var flag = true,
			mouseY = 0,
			topValue = 0;
	...
	} else if ("pt_lightbox_trigger_pageload" == $trigger) {
		if ($display_after === 0) {
			$.removeCookie($popup_id, { path: "/" });
		}
		if (!$.cookie($popup_id)) {
		...
		if (
			...
			$trigger_element = ".pt-modal-popup-link";
		}
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-lightbox.default",
		LightBox
	);
});
...
var LogoCarouselHandler = function($scope, $) {
	var $carousel = $scope.find(".pt-logo-carousel").eq(0),
		$items =
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-logo-carousel.default",
		LogoCarouselHandler
	);
});
...
var MailChimp = function($scope, $) {
	var $mailChimp = $scope.find(".pt-mailchimp-wrap").eq(0),
		$mailchimp_id =
	...
	pt_mailchimp_subscribe(
		"pt-mailchimp-form-" + $mailchimp_id + "",
		$api_key,
		$list_id,
		$button_text,
		$success_text,
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-mailchimp.default",
		MailChimp
	);
});
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-offcanvas.default",
		ptOffcanvas
	);
});
...
var PostCarouselHandler = function($scope, $) {
	var $postCarousel = $scope.find(".pt-post-carousel").eq(0),
		$autoplay =
	...
	var ptPostCarousel = new Swiper($postCarousel, {
		direction: "horizontal",
		speed: $speed,
		effect: $effect,
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-post-carousel.default",
		PostCarouselHandler
	);
});
...
var postListHandler = function ($scope, $) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				if ($content.hasClass("no-posts-found") || $content.length == 0) {
					...
					$(".pt-post-appender", $scope).empty().append($content);
	...
	// var $_this = $scope.find(".pt-post-list-container");
	// var advanceLayout = $scope.find(
	// 	".pt-post-list-container.layout-advanced"
	// );
	...
	// 	insMaxHeight(".pt-post-list-title");
	// }
};

jQuery(window).on("elementor/frontend/init", function () {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-post-list.default",
		postListHandler
	);
});
...
/* 37. Section Parallax
/*=================================*/
var ptParallaxHandler = function($scope, $) {
    var target = $scope,
        sectionId = target.data("id"),
        settings = false,
        editMode = elementorFrontend.isEditMode();
    ...
    function generateEditorSettings(targetId) {
        ...
        if (!sectionData.hasOwnProperty("pt_parallax_type")) {
            return false;
        }

        if ("" == sectionData["pt_parallax_type"]) {
            return false;
        }

        if (
            "multi" != sectionData["pt_parallax_type"] &&
            "automove" != sectionData["pt_parallax_type"]
        ) {
            settings.push(sectionData["pt_parallax_switcher"]);
            settings.push(sectionData["pt_parallax_type"]);
            settings.push(sectionData["pt_parallax_speed"]);
            settings.push(
                "yes" == sectionData["pt_parallax_android_support"] ? 0 : 1
            );
            settings.push(
                "yes" == sectionData["pt_parallax_ios_support"] ? 0 : 1
            );
            settings.push(sectionData["pt_parallax_background_size"]);
            settings.push(sectionData["pt_parallax_background_pos"]);
        } else if ("automove" == sectionData["pt_parallax_type"]) {
            settings.push(sectionData["pt_parallax_switcher"]);
            settings.push(sectionData["pt_parallax_type"]);
            settings.push(sectionData["pt_auto_speed"]);
            settings.push(sectionData["pt_parallax_auto_type"]);
        } else {
            if (!sectionData.hasOwnProperty("pt_parallax_layers_list")) {
                return false;
            }

            sectionMultiData = sectionData["pt_parallax_layers_list"].models;
            ...
            settings.push(sectionData["pt_parallax_type"]);
            settings.push(
                "yes" == sectionData["pt_parallax_layer_invert"] ? 1 : 0
            );
    ...
    function generateMultiLayers() {
        ...
        $.each(settings, function(index, layout) {
            if (2 < index) {
                if (
                    null != layout["pt_parallax_layer_image"]["url"] &&
                    "" != layout["pt_parallax_layer_image"]["url"]
                ) {
                    if (
                        "yes" == layout["pt_parallax_layer_mouse"] &&
                        "" != layout["pt_parallax_layer_rate"]
                    ) {
                    ...
                    var backgroundImage =
                            layout["pt_parallax_layer_image"]["url"],
                        $html = $(
                            '<div id="pt-parallax-layer-' +
                                counter +
                                '"' +
                                mouseParallax +
                                mouseRate +
                                ' class="pt-parallax-layer"></div>'
                        )
                            .prependTo(target)
                            .css({
                                "z-index":
                                    layout["pt_parallax_layer_z_index"],
                                "background-image":
                                    "url(" + backgroundImage + ")",
                                "background-size":
                                    layout["pt_parallax_layer_back_size"],
                                "background-position-x":
                                    layout["pt_parallax_layer_hor_pos"] + "%",
                                "background-position-y":
                                    layout["pt_parallax_layer_ver_pos"] + "%"
                            });
        ...
        target.mousemove(function(e) {
            $(this)
                .find('.pt-parallax-layer[data-parallax="true"]')
...
/*=================================*/
var ptParticlesHandler = function ($scope, $) {
    var sectionId = $scope.data('id'),
        ...
        var preset_theme = $scope.data('preset_theme'),
            ...
            source = $scope.data('pt_ptheme_source'),
            settings;
    ...
    $scope.addClass('pt-particles-section');
    ...
    if (window.isEditMode) {
        ...
        settings.switch = particleArgs['pt_particle_switch'];
        settings.themeSource = particleArgs['pt_particle_theme_from'];

        if (settings.themeSource == 'presets') {
            settings.selected_theme = (localize.ParticleThemesData[particleArgs['pt_particle_preset_themes']]);
        }

        if ((settings.themeSource == 'custom') && ('' !== particleArgs['pt_particles_custom_style'])) {
            settings.selected_theme = particleArgs['pt_particles_custom_style'];
        }
        ...
        particlesJS('pt-section-particles-'+sectionId, themes);
    }
    ...
    if (settings.switch == 'yes') {
        ...
        if (settings.themeSource === 'presets' || settings.themeSource === 'custom' && '' !== settings.selected_theme) {
            $scope.attr('id', 'pt-section-particles-' + sectionId);
            if (typeof particlesJS !== 'undefined' && $.isFunction(particlesJS)) {
                particlesJS("pt-section-particles-" + sectionId, JSON.parse(settings.selected_theme));
        ...
        $scope.removeClass('pt-particles-section');
    }

};

jQuery(window).on('elementor/frontend/init', function () {
    elementorFrontend.hooks.addAction( 'frontend/element_ready/section', ptParticlesHandler );
});
...
var TeamMemberCarouselHandler = function($scope, $) {
    var $carousel = $scope.find(".pt-tm-carousel").eq(0),
        $pagination =
            $carousel.data("pagination") !== undefined
                ? $carousel.data("pagination")
                : ".swiper-pagination",
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/pt-team-member-carousel.default",
...
var TestimonialSliderHandler = function($scope, $) {
	var $testimonialSlider = $scope.find(".pt-testimonial-slider-main").eq(0),
		$pagination =
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-testimonial-slider.default",
...
var ToggleHandler = function($scope, $) {
	var toggle_elem = $scope.find(".pt-toggle-container").eq(0);
	$(toggle_elem).each(function() {
		var $toggle_target = $(this).data("toggle-target");
		var $toggle_switch = $($toggle_target).find(".pt-toggle-switch");
		$($toggle_target)
			.find(".pt-primary-toggle-label")
			.addClass("active");
		$($toggle_switch).toggle(
			function() {
				...
				$($parent_container)
					.find(".pt-toggle-content-wrap")
					.removeClass("primary");
				$($parent_container)
					.children(".pt-toggle-content-wrap")
					.addClass("secondary");
				$($parent_container)
					.find(".pt-toggle-switch-container")
					.addClass("pt-toggle-switch-on");
				$(this)
					.parent()
					.parent()
					.find(".pt-primary-toggle-label")
				...
				$(this)
					.parent()
					.parent()
					.find(".pt-secondary-toggle-label")
					.addClass("active");
			},
			function() {
				var $parent_container = $(this).closest(
				...
				$($parent_container)
					.find(".pt-toggle-switch-container")
					.removeClass("pt-toggle-switch-on");
				$(this)
					.parent()
					.parent()
					.find(".pt-primary-toggle-label")
...
jQuery(window).on("elementor/frontend/init", function() {
	elementorFrontend.hooks.addAction(
		"frontend/element_ready/pt-toggle.default",
		ToggleHandler
	);
});

var TwitterFeedCarouselHandler = function($scope, $) {
    var $carousel = $(".pt-twitter-feed-carousel", $scope),
        $pagination =
            $carousel.data("pagination") !== undefined
                ? $carousel.data("pagination")
                : ".swiper-pagination",
...
jQuery(window).on("elementor/frontend/init", function() {
    elementorFrontend.hooks.addAction(
        "frontend/element_ready/pt-twitter-feed-carousel.default",
...
(function ($) {
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/pt-testimonials-slider1.default', PTTestimonialsSlider);
    });
