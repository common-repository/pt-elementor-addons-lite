...
use Pt_Addons_Elementor\Classes\PT_Installer;
...
class PT_Notice {
	 ...
	 * @var array
	 */
	const ADMIN_UPDATE_NOTICE_KEY = 'pt_seen_notices';
	public $text_domain           = 'pt-text-domain-notice';
	/**
	 * All Data
	 *
	 * @var array
	...
	public function __construct( $plugin_file = '', $version = '' ) {
		...
		$this->notice_id        = 'pt_notice_' . str_replace( '.', '_', $this->version );
		$this->do_notice_action = 'pt_notices_for_' . $this->plugin_name;
		new PT_Installer( $this->plugin_name );
	}
	/**
	 * Initiate The Plugin
	 *
	...
	public function hooks() {
		add_action( 'pt_notice_clicked_for_' . $this->plugin_name, array( $this, 'clicked' ) );
		add_action( 'wp_ajax_pt_upsale_notice_dissmiss_for_' . $this->plugin_name, array( $this, 'upsale_notice_dissmiss' ) );
		add_action( 'wp_ajax_pt_notice_dissmiss_for_' . $this->plugin_name, array( $this, 'notice_dissmiss' ) );
		add_action( 'pt_before_notice_for_' . $this->plugin_name, array( $this, 'before' ) );
		add_action( 'pt_after_notice_for_' . $this->plugin_name, array( $this, 'after' ) );
		add_action( 'pt_before_upsale_notice_for_' . $this->plugin_name, array( $this, 'before_upsale' ) );
		add_action( 'pt_after_upsale_notice_for_' . $this->plugin_name, array( $this, 'after' ) );
		add_action( $this->do_notice_action, array( $this, 'content' ) );
		if ( current_user_can( 'install_plugins' ) ) {
			if ( isset( $_GET['plugin'] ) && $_GET['plugin'] == $this->plugin_name ) {
				do_action( 'pt_notice_clicked_for_' . $this->plugin_name );
				/**
	...
	public function before() {
		...
		echo '<div class="' . $classes . ' pt-' . $current_notice . '-notice">';
	}
	/**
	 * After Notice
	 *
	...
	public function content() {
		...
		switch ( $notice ) {
			case 'opt_in':
				do_action( 'pt_optin_notice_for_' . $this->plugin_name );
				break;
			case 'first_install':
				if ( $options_data[ $this->plugin_name ]['first_install'] !== 'deactivated' ) {
					do_action( 'pt_first_install_notice_for_' . $this->plugin_name );
					$this->get_thumbnail( 'first_install' );
					$this->get_message( 'first_install' );
				}
				break;
			case 'update':
				do_action( 'pt_update_notice_for_' . $this->plugin_name );
				$this->get_thumbnail( 'update' );
				$this->get_message( 'update' );
				$this->dismiss_button_scripts();
				break;
			case 'review':
				do_action( 'pt_review_notice_for_' . $this->plugin_name );
	...
	public function before_upsale() {
		...
		echo '<div class="error notice is-dismissible pt-upsale-notice ' . $classes . '">';
	}
	/**
	 * Upsale Notice
	 */
	public function upsale_notice() {
		do_action( 'pt_before_upsale_notice_for_' . $this->plugin_name );
			do_action( 'pt_upsale_notice_for_' . $this->plugin_name );
			$this->get_thumbnail( 'upsale' );
			$this->get_message( 'upsale' );
		do_action( 'pt_after_upsale_notice_for_' . $this->plugin_name );
	...
	protected function get_thumbnail( $msg_for ) {
		...
		if ( isset( $this->data['thumbnail'] ) && isset( $this->data['thumbnail'][ $msg_for ] ) ) {
			$output      = '<div class="pt-notice-thumbnail">';
	...
	protected function get_message( $msg_for ) {
		if ( isset( $this->data['message'] ) && isset( $this->data['message'][ $msg_for ] ) ) {
			echo '<div class="pt-notice-message">';
	...
	public function admin_notices() {
		...
		do_action( 'pt_before_notice_for_' . $this->plugin_name );
			do_action( $this->do_notice_action );
		do_action( 'pt_after_notice_for_' . $this->plugin_name );
	}
	/**
	...
	public function dismissible_notice( $links_for = '' ) {
		...
		if ( $links ) :
			$output = '<ul class="pt-notice-link">';
				...
				if ( ! empty( $link_value['label'] ) ) {
					...
					if ( isset( $link_value['link'] ) ) {
						...
						if ( isset( $link_value['data_args'] ) && is_array( $link_value['data_args'] ) ) {
							...
							$link                = wp_nonce_url( $normal_link, 'pt-nonce' );
						}
						$class = '';
	...
	protected function get_options_data( $key = '' ) {
		$options_data = get_option( 'pt_plugins_data' );
		if ( empty( $key ) ) {
			return $options_data;
		}
	...
	protected function update_options_data( $args = array() ) {
		...
		update_option( 'pt_plugins_data', $options_data );
	}
	/**
	...
	public function notice_dissmiss() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), 'pt_notice_dissmiss' ) ) {
			return;
		}
		if ( ! isset( $_POST['action'] ) || ( $_POST['action'] !== 'pt_notice_dissmiss_for_' . $this->plugin_name ) ) {
			return;
		}
	...
	public function upsale_notice_dissmiss() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['_wpnonce'] ) ), 'pt_upsale_notice_dissmiss' ) ) {
			return;
		}
		if ( ! isset( $_POST['action'] ) || ( $_POST['action'] !== 'pt_upsale_notice_dissmiss_for_' . $this->plugin_name ) ) {
			return;
		}
		$dismiss = isset( $_POST['dismiss'] ) ? $_POST['dismiss'] : false;
		if ( $dismiss ) {
					...
					if( $('.notice').find('.notice-dismiss').length > 0 ) {
						$('.notice').on('click', 'button.notice-dismiss', function (e) {
							...
							$.ajax({
								...
								type: 'post',
								data: {
									action: 'pt_notice_dissmiss_for_<?php echo $this->plugin_name; ?>',
									_wpnonce: '<?php echo wp_create_nonce( 'pt_notice_dissmiss' ); ?>',
									dismiss: true,
			...
			jQuery(document).ready( function($) {
				...
				$('#plugin-install-core-<?php echo $this->plugin_name; ?>').on('click', function (e) {
					...
					$.ajax({
						...
						type: 'POST',
						data: {
							action: 'pt_upsale_core_install_<?php echo $this->plugin_name; ?>',
							_wpnonce: '<?php echo wp_create_nonce( 'pt_upsale_core_install_' . $this->plugin_name ); ?>',
				...
				<?php endif; ?>
				$('.pt-upsale-notice').on('click', 'button.notice-dismiss', function (e) {
					e.preventDefault();
					console.log( e );
					return;
					$.ajax({
						...
						type: 'post',
						data: {
							action: 'pt_upsale_notice_dissmiss_for_<?php echo $this->plugin_name; ?>',
							_wpnonce: '<?php echo wp_create_nonce( 'pt_upsale_notice_dissmiss' ); ?>',
							dismiss: true
						},
						success: function(response) {
							console.log('Successfully saved!');
